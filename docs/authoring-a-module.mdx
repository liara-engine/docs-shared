...
`/<repo>/latest/` redirect. Two schema versions are still read side by side;
the navbar detects which one it is reading from the presence of
`manifest_version`.

<Aside type="caution" title="v1 is deprecated">
The v1 manifest is deprecated. It is still read, but support for it ends at
**v0.1** — the milestone the meta repository tags `v1.0.0` — and a module
still on v1 stops building then. Migrating is four edits: §2.1.
</Aside>

<Tabs syncKey="manifest">
<TabItem label="v2 — current">
...
Documented here because modules and published tags still carry it, not because
it is a choice — see §2.1.
...
<Aside type="danger" title="v1 has a deadline">
v1 is read until **v0.1** (`liara` tagged `v1.0.0`) and not after. Until then
nothing breaks and a v1 manifest keeps building; after it, a build that finds
one fails. New modules write v2 — it is stricter, it carries `kind`, and it
can annotate a version — and existing ones have §2.1.
</Aside>
...
### 2.1 Migrating from v1 to v2

Four edits, none of which touches a published URL — a deployed version keeps
the manifest it was built with until someone rebuilds it.

| v1                               | v2                                                                  |
|----------------------------------|---------------------------------------------------------------------|
| `…/module-manifest.schema.json`  | `…/module-manifest-v2.schema.json`                                  |
| —                                | `"manifest_version": 2`                                             |
| —                                | `"kind"`, which supersedes the registry's `is_abi` and `meta` flags |
| —                                | `metadata.repo`, the GitHub repository name                         |
| `"abi_compatibility": ["1.0.0"]` | `"abi": "1.0.0"`, or an array for several anchors                   |

`kind` is the only one that needs a decision; the **v2** tab above lists the
three values and what each does to `versions`. A `contract` manifest then drops
`abi` — its own versions *are* the ABI — and so does an `infrastructure` one,
which has no ABI relation at all; a `module` or `host` manifest must carry it
on every version.

```json title="manifest.json" {2,3,4,8,12,13}
{
  "$schema": "https://liara-engine.github.io/liara/schemas/module-manifest-v2.schema.json",
  "manifest_version": 2,
  "kind": "module",
  "metadata": {
    "name": "Liara Core",
    "description": "The engine's core module.",
    "repo": "liara-core",
    "latest": "1.0.0"
  },
  "versions": {
...
The highlighted lines are the ones that changed. Nothing else in the module is
affected — the pages, the `Doxyfile` and the CI caller stay as they are, and
the next build publishes from the new manifest.

<Aside type="caution" title="Old tags do not migrate themselves">
`rebuild-version.yml` reads the manifest from the tag it is rebuilding, so an
old tag keeps its v1 manifest forever. Rebuild whatever you still want
refreshed — an old release that should pick up a theme change, say — while
both formats are read
([CI and deployment](../ci-and-deployment/#4-rebuilding-versions)).
</Aside>
...
Whatever it finds is filed by kind, into one URL segment and one sidebar
section apiece:

| Section | Holds | URL |
| --- | --- | --- |
| Headers | one page per file | `api/headers/<file>/` |
| Namespaces | a page per namespace that has symbols of its own | `api/namespaces/<name>/` |
| Classes and structs | `class`, `struct`, `union`, `interface` | `api/classes/<name>/` |
| Concepts | C++20 concepts | `api/concepts/<name>/` |

A section with nothing in it does not appear — a C module never shows an empty
"Namespaces". A namespace whose only contents are *other* namespaces gets no
page either: it appears in the namespace tree on the section's own index page,
where its nesting is the whole of what there is to say about it.

The vocabulary follows the language. A C `struct` has **Fields**; a class, or
a `struct` with member functions, templates or a namespace-qualified name, has
**Methods** and **Attributes** — so a member never reads as though it were a
free function or a global.
...
Everything else Doxygen calls a section — `@note`, `@warning`, `@attention`,
`@pre`, `@post`, `@invariant`, `@since`, `@see`, `@remark`, `@deprecated`,
`@todo`, `@bug`, `@par` — becomes a labelled block where you wrote it, with a
colour and a glyph for what it promises: pink for a contract you have to hold
up, blue for an aside, amber and red for the two levels of "careful", green
for a `@todo`, grey for bookkeeping.

<Aside type="note" title="Why not Starlight's callouts?">
`@note`, `@warning` and `@attention` used to be rendered as Starlight asides,
which are a good deal larger than these blocks. Two things came of it: one
symbol's documentation spoke two visual languages, a `@pre` sitting three
lines above an `@attention` that looked nothing like it; and since Starlight
has four callout types against Doxygen's dozen sections, `@warning` and
`@attention` both landed on `caution` and became indistinguishable. Asides are
still asides in *your prose* — this is only about generated pages.
</Aside>
...
1. Add a v2 `manifest.json` — `manifest_version`, `kind`, `metadata.repo`,
   `metadata.latest`, and at least a `dev` version.
