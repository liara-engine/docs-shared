...
- astro/ **the shared Starlight preset — `@liara/starlight-preset`**
  ...
  - components/
    - SiteTitle.astro module and version switchers
...
flowchart LR
    ...
    M -->|"api/pages.mjs"| P["page HTML + headings"]
    P -->|"api/loader.mjs"| S["the `docs` collection<br/>(rendered.html pre-populated)"]
    S --> R["/&lt;repo&gt;/&lt;version&gt;/api/…"]
```

Three decisions in there are worth knowing about, because they are not
...
3. **Layout is chosen per module.** `split: 'file'` gives one page per header —
...
Generated pages are highlighted by Shiki using a theme whose colours are
`var(--liara-code-*)` references rather than literals, so light and dark
switching is free and the project's own nine-role code palette stays in use.
Prose code fences go through Expressive Code, as everywhere else in Starlight.
...
3. **Extract the API surface.** If there is a `Doxyfile` and `--skip-api` was
   ...
   `/working/build/xml`, and fail if it produces no `index.xml`.
...
6. **Prune search.** A published version builds no index at all; a preview
   keeps its own, minus the Pagefind UI bundles Starlight never fetches
   ([Search](../search/)).
...
| `/<repo>/<version>/api/…` | Generated API reference |
